...
export default function HeroBanner() {
  return (
    <section
      ...
      style={{ background: 'linear-gradient(135deg, #F97316 0%, #EA580C 50%, #9A3412 100%)' }}
    >
      ...
      <h1 className="mb-1.5 text-[26px] font-black leading-tight tracking-tight text-white">
        Os melhores<br />achadinhos pra você
      </h1>
...
export default function CategoryForm({ categoria, onSave, onCancel }: CategoryFormProps) {
  ...
  return (
    <form onSubmit={handleSubmit} className="flex flex-col gap-4">
      ...
      <div>
        <Label>Slug</Label>
        <Input value={slug} onChange={e => setSlug(e.target.value)} required readOnly={isEdit} className={isEdit ? 'bg-gray-50' : ''} />
      </div>
      <div className="grid grid-cols-2 gap-4">
        <div>
          <Label>Emoji</Label>
        ...
        <div>
          ...
          <div className="flex gap-2">
            <input type="color" value={cor} onChange={e => setCor(e.target.value)} className="h-10 w-12 cursor-pointer rounded border" />
    ...
    <div>
      ...
      <Dialog open={!!editando} onOpenChange={() => setEditando(null)}>
        <DialogContent className="max-w-lg">
          ...
          {editando && (
            <CategoryForm categoria={editando} onSave={handleSaved} onCancel={() => setEditando(null)} />
          )}
        </DialogContent>
      </Dialog>
